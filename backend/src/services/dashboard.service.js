...
export const getUserStats = async (userId) => {
    // 사용할 쿼리들 
    const q1 = `SELECT COUNT(*) as todayCount FROM study_logs WHERE user_id = ? AND DATE(CONVERT_TZ(created_at, '+00:00', '+09:00')) = DATE(CONVERT_TZ(NOW(), '+00:00', '+09:00'))`;
    ...
    const q6 = `
        SELECT 
            DATE_FORMAT(CONVERT_TZ(created_at, '+00:00', '+09:00'), '%m-%d') as date, 
            COUNT(*) as count 
        FROM study_logs 
        WHERE user_id = ? 
    ...
    return {
        ...
        activityLog: res6[0] || [],
        categories: res7[0], // [{category: 'SNS', total: 20, mastered: 15}, ...]
        calendarDays: res8[0].map(r => Number(r.day))
  };
};
