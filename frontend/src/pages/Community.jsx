...
export default function Community() {
  ...
  const handleWriteSubmit = async (titleInput, contentInput) => {
    try {
        ...
        const response = await fetch('http://localhost:8000/api/board/write', {
            method: 'POST',
            headers: {
  ...
  const handleReaction = async (id, type) => {
    try {
        const response = await fetch(`http://localhost:8000/api/board/${id}/${type}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
  ...
  const handleCommentSubmit = async (postId, commentContent) => {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/board/${postId}/comment`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
