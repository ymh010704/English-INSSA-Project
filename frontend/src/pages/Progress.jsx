...
const CATEGORY_META = {
  SNS:      { label: "SNS / 일상",    color: G.accent,   icon: "📱" },
  React:    { label: "감탄 / 리액션", color: G.blue,     icon: "😮" },
  Praise:   { label: "칭찬 / 긍정",   color: G.green,    icon: "🙌" },
  Refusal:  { label: "완곡 / 거절",   color: G.purple,   icon: "🙅" },
  GenZ:     { label: "Gen Z 유행어",  color: "#f59e0b",  icon: "✨" },
  Romance:  { label: "연애 / SNS",    color: "#ec4899",  icon: "💕" },
  Etc:      { label: "기타",          color: G.gray,     icon: "📝" },
};

const FALLBACK_COLORS = [G.accent, G.blue, G.green, G.purple, "#f59e0b", "#ec4899", G.gray];
...
function buildWeekly(activityLog) {
  ...
  return KOR.map((d, i) => {
    ...
    return { d, count: found ? Number(found.count) : 0 };
  });
}

function getCalendarData(calendarDays) {
  const now = new Date();
  ...
  return { year, month, startDay, days, today, studied: calendarDays };
}

const BADGES = [
  { emoji: "🔥", name: "7일 연속",      desc: "7일 연속 학습"          },
  { emoji: "📚", name: "첫 10개",       desc: "표현 10개 학습"         },
  { emoji: "⚡", name: "스피드 러너",   desc: "하루 10개 학습"         },
  { emoji: "🌙", name: "야간 학습",     desc: "밤 11시 이후 학습"      },
  { emoji: "🏆", name: "퍼펙트 스코어", desc: "연습 100% 달성"         },
  { emoji: "👑", name: "30일 연속",     desc: "30일 연속 학습"         },
  { emoji: "🔥", name: "올라운더",      desc: "모든 카테고리 50% 이상" },
  { emoji: "💎", name: "100개 달성",    desc: "표현 100개 학습"        },
  { emoji: "🤖", name: "AI 마스터",     desc: "AI 대화 50회"           },
];
...
function StreakCard({ streak }) {
  return (
    <div style={{ background: `linear-gradient(145deg, ${G.navy}, #1e3a5f)`, borderRadius: 24, padding: "28px 30px", border: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", fontFamily: "'Noto Sans KR', sans-serif" }}>최장 기록 <span style={{ color: G.accent2, fontWeight: 700 }}>21일</span> · 오늘도 학습하면 <span style={{ color: G.green, fontWeight: 700 }}>{streak + 1}일</span></div>
    </div>
  );
}
...
function CategoryChart({ categories }) {
  if (!categories.length) return (
    <Section title="📊 카테고리별 숙련도" sub="카테고리별 학습 완료 비율">
      <div style={{ color: G.gray, fontSize: 13, textAlign: "center", padding: "20px 0" }}>학습 데이터가 없습니다</div>
    </Section>
  );
  return (
    <Section title="📊 카테고리별 숙련도" sub="카테고리별 학습 완료 비율">
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {categories.map((c, i) => {
          const meta = CATEGORY_META[c.category] || { label: c.category, color: FALLBACK_COLORS[i % FALLBACK_COLORS.length], icon: "📚" };
            ...
            <div key={c.category}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 500, color: G.black }}>
                  ...
                  <span style={{ fontSize: 11, color: G.gray }}>({c.mastered}개)</span>
                </div>
              ...
              <div style={{ height: 8, background: G.lightGray, borderRadius: 100, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${pct}%`, background: meta.color, borderRadius: 100, transition: "width 1s ease" }} />
              </div>
            </div>
          );
        })}
...
function MonthlyCalendar({ calendarDays }) {
  const { year, month, startDay, days, studied, today } = getCalendarData(calendarDays);
...
function BadgeGrid({ badges }) {
  const done = BADGES.filter(b => badges.includes(b.name));
  const locked = BADGES.filter(b => !badges.includes(b.name));
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      axios.get("/api/dashboard/stats", { headers: { Authorization: `Bearer ${token}` } })
        .then(r => {
          setXp(Number(r.data.xp) || 0);
          setStreak(Number(r.data.streak) || 0);
          setTodayCount(Number(r.data.todayCount) || 0);
          setMasteredCount(Number(r.data.masteredCount) || 0);
          setActivityLog(r.data.activityLog || []);
          setCategories(r.data.categories || []);
          setCalendarDays(r.data.calendarDays || []);
        }),
    ...
    <div style={{ minHeight: "100vh", background: G.lightGray, fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
          {[
            { icon: "📚", label: "총 학습 표현", value: `${todayCount}개`, color: G.accent },
            { icon: "✅", label: "완료한 카드", value: `${masteredCount}개`, color: G.green },
            { icon: "🔥", label: "연속 학습일", value: `${streak}일`, color: G.blue },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          <WeeklyChart weekly={buildWeekly(activityLog)} />
          <CategoryChart categories={categories} />
        </div>

        {/* 캘린더 + 뱃지 */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          <MonthlyCalendar calendarDays={calendarDays} />
